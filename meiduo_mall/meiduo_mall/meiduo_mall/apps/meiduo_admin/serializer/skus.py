...
class SKUSerializer(serializers.ModelSerializer):
    ...
    #         # SKU商品的规格数据是否一致
    #         specs_ids = [spec.get('spec_id') for spec in specs]
    #         spu_specs_id = [spec.id for spec in spu_specs]
    #
    #         if specs_ids.sort() != spu_specs_id.sort():
    #             raise serializers.ValidationError('SKU规格不完整')
    #
    #         for spec in specs:
    #             spec_id = spec.get('spec_id')
    #             option_id = spec.get('option_id')
    ...
    def update(self, instance, validated_data):
        ...
        with transaction.atomic():
            # 调用父类方法更新sku商品  不需要修改更新的操作 我们要王丽加上校验的逻辑
            sku = super().update(instance=instance, data=validated_data)
            # 清除sku原有的规格信息
